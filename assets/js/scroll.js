 ...
 * 🎯 Why: Previously, setActive iterated over all links with an O(N) loop on every IntersectionObserver callback. Also used `document.querySelector` instead of `getElementById`. Processing updates in a loop caused layout thrashing.
 * 📊 Impact: Changes O(N) link iteration loop to O(1) array lookup. Reduces DOM lookups during initial load. Fixes redundant section observing. Prevents layout thrashing by applying DOM mutations once per scroll event.
 */
(() => {
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      // Find the last intersecting entry to prevent multiple sequential DOM updates
      // during fast scrolling (which causes layout thrashing).
